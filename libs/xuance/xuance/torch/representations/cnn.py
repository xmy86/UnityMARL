...
class Basic_CNN(Module):
    ...
    def forward(self, observations: np.ndarray):
        ...
        known_shape = (5, 40, 40)
        total_elements = observations.size
        first_dim = total_elements // np.prod(known_shape)
        tensor_observation = torch.as_tensor(np.transpose(observations.reshape(first_dim, 5, 40, 40), (0, 3, 1, 2)), dtype=torch.float32,
                                             device=self.device)
