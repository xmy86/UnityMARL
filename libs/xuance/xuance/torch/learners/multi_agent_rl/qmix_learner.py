...
class QMIX_Learner(LearnerMAS):
    def __init__(self,
        ...
        self.n_actions = {k: self.policy.action_space[k].shape[0] for k in self.model_keys} # self.policy.action_space[k].n
    ...
    def update(self, sample):
        ...
        for key in self.model_keys:
            actions_indices = actions[key].argmax(dim=-1)
            q_eval_a[key] = q_eval[key].gather(-1, actions_indices.long().unsqueeze(-1)).squeeze(-1)
